=== BizPress ===
Contributors: Bizink,Jayden Major
Tags: Bizink, BizPress, business content, marketing
Requires at least: 6.0
Tested up to: 6.9.4
Stable tag: 2.0.1
Requires PHP: 8.1

Display business content on your website that is automatically updated by the Bizink team.

== Description ==
Display business content on your website that is automatically updated by the Bizink team.

=== BizPress Plugins ===
- BizPress Accounting Glossary
- BizPress Business Terms Glossary
- BizPress Key Dates
- BizPress Xero Resources
- BizPress Business Resources
- BizPress Payroll
- BizPress Luca
- BizPress Resources - WIP
- BizPress XPM (Forms) - WIP

== Changelog ==

= 2.0.1 =
* PHP Supported Version

= 2.0.0 =
* More Keydates Changes

= 1.9.9 =
* Update Keydates
* Update PHP Requirements

= 1.9.8 =
* Update Checking

= 1.9.7 =
* Remove the Word test from Keydates

= 1.9.6 =
* Remove Analytics
* Keydates One Page

= 1.9.5 =
* Fix Filters

= 1.9.4 =
* Resources Fix

= 1.9.3 =
* SEO Settings

= 1.9.2 =
* Admin Script URL Fix - CXBPC_URL undefined

= 1.9.1 =
* Region Fix for Resources
* Clear Cahce Fixes

= 1.9 =
* New Card Fix
* BizPress Resources

= 1.8.9 =
* New Fix for Meta Description
* New Card Fix

= 1.8.8 =
* Better Glossary Navigation
* Fix for no Meta Description

= 1.8.7 =
* New Layout for Glossary
* Remove Old Font Awesome css and fonts
* New Icons accross BizPress
* Updater Fix
* Fix Warnings
* Bug Fix
* Updater Fix

= 1.8.6 =
* Css for Calculator

= 1.8.5 =
* SEO Non-Conical URL Fix 2

= 1.8.4 =
* Version Bump

= 1.8.3 =
* Open Graph SEO Meta Description Fix

= 1.8.2 =
* SEO Meta Description Fix

= 1.8.1 =
* SEO Non-Conical URL Fix

= 1.8 =
* Publish Status

= 1.7.9 =
* Added Support for FreshBooks Resources

= 1.7.8 =
* Debug Fix


= 1.7.7 =
* Better Shortcode Support

= 1.7.6 =
* Fix Spelling Mistake

= 1.7.5 =
* Bug Fix: SEO Titles

= 1.7.4 =
* Adding Content Filters
* Added support to better handle seo for the Bizpress Content

= 1.7.3 =
* Fix for overuse of Analytics Requests
* Fix for shortcode not loading page content

= 1.7.2 =
* Fix for Blogs Products
* Paid Bizpress Blogs Update

= 1.7.1 =
* Filter Url's
* Fix for Analytics

= 1.7 =
* Fix Permission Issue with the Bizink Login

= 1.6.9 =
* Remove Bizink Login Admin Message

= 1.6.8 =
* Fixes an issue with some videos not showing

= 1.6.7 =
* Fixed an issue with the Sage Resources title not displaing

= 1.6.6 =
* Support for sorting Sage Resources

= 1.6.5 =
* Added support for Sage Resources

= 1.6.4 =
* Fix for width of the Resources page

= 1.6.3 =
* Fix for Quickbooks thumbnail images

= 1.6.2 =
* Added Support for Pagnation and Icons from Topics

= 1.6.1 =
* Bug fix: Fixing URL Rewrite for Business Content

= 1.6.0 =
* Default Image for posts that don't have a featured image set.
* Bug fix for Business Content Topics

= 1.5.9 =
* Fix for some iFrame height issues

= 1.5.8 =
* Missing ?

= 1.5.7 =
* General Bug Fixes
* Calculator Fix
* Hide Label Fix

= 1.5.6 =
* Fix iframe height

= 1.5.5 =
* Fix iframe border

= 1.5.4 =
* Landing Pages

= 1.5.3 =
* API Update
* Analytics Update

= 1.5.2 =
* Fix PHP Warnings

= 1.5.1 =
* Better Login Screen
* Addon Installer
* Added switches in options menu
* Added a page creation feature

= 1.5 =
* HOT Fix: Fix a Bug individual Page loading

= 1.4.9 =
* HOT Fix: Fix a Bug in the helpers file the prevents pages from loading

= 1.4.8 =
* Handeling of different bizpress levels

= 1.4.7 =
* Better fix for page titles

= 1.4.6 =
* Fix for MYOB Content

= 1.4.5 =
* HotFix: Shortcode page loading

= 1.4.4 =
* Added Img ALT Tags
* Added Sitemap XML with yeost integration

= 1.4.3 =
* BizPress Anylitics URL Update

= 1.4.2 =
* Support for Bizpress Anylitics
* Removed old Anylitics
* Update message about subscription expireing

= 1.4.1 =
* Added Irland Region

= 1.4 =
* Key Update

= 1.3.9 =
* MYOB Term Sorting

= 1.3.8 =
* Bizpress Luca 2 Auto login fix

= 1.3.7 =
* Luca Template fix

= 1.3.6 =
* Keydates HotFix

= 1.3.5 =
* Luca 2 & Other theme Compatibality
* Keydate now only displays current Finincial Year
* Payroll Glossary
* Payroll Resources

= 1.3.4 =
* HotFix - Glossary

= 1.3.3 =
* Added the Bizpress Glossary

= 1.3.2 =
* Updateing the plugin updater

= 1.3.2 =
* Keydates Update

= 1.3.1 =
* Combine different Key Dates plugins into one

= 1.3 =
* Bug Fix: Fixed and issue with Quickbooks Recources

= 1.2.9 =
* Bug Fix: Xero Recources
* New: Added mode for no conflict url setting. Config BIZINK_NOCONFLICTURL define(BIZINK_NOCONFLICTURL, true); in wp_config.php 

= 1.2.8 =
* New Feature: Cacheing of pages requests
* General Compatibality updates

= 1.2.7 =
* Bug Fixes

= 1.2.6 =
* Add: a hideing feature
* Fix a but that enables better support for PHP 8.1

= 1.2.5 =
* Fix the the bizpress content issues
* Fixed a version 1.0 vs 1.1 api request issue

= 1.2.4 =
* Fixed and error that ocours when the plugin dosn't get a responce from the server.

= 1.2.3 =
* Added [bizpress-content] & [bizpress-landing] shortcodes they will map to the same as [bizink-content] & [bizink-landing] respectively
* Added extra fields to the BizPress support form
* You can now Copy the shortcode form the bizpress settings pages with a copy button - Both for Main Page and Landing Page shortcodes
* Pages that have the [bizpress-content] or [bizink-content] shortcodes but haven't be set to show anything in the Bizpress will now show nothing for the bizpress content. (User content remains unchanged)

= 1.2.2 =
* Change some of the frontend styleing for the Xero content.
* Bug Fix: Fixed a but that show Undefined variable: post_count

= 1.2.1 =
* Added the Landing Page section to the settings tab with the ability to copy the shortcode

= 1.2 =
* Changed the version number in the readme
* Added support form to the support tab

= 1.1 =
* Added other Plugins to the readme
* Added the support tab to the settings

= 1.0 =
* Bizpress launch

= 0.9 =
* Pre Realease